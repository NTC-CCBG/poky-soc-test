...
/*
 * Nuvoton NCT6694 wdt adapter driver based on USB interface.
 *
...
static int nct6694_wdt_start(struct watchdog_device *wdev)
{
	...
	struct nct6694 *chip = wdt->data->chip;
...
static int nct6694_wdt_stop(struct watchdog_device *wdev)
{
	...
	ret = nct6694_setusb_wdata(chip, REQUEST_WDT_MOD, REQUEST_WDT_CMD1_OFFSET(wdt->wdev_idx), REQUEST_WDT_CMD1_LEN, buf);
	if (ret) {
...
static int nct6694_wdt_set_timeout(struct watchdog_device *wdev, unsigned int timeout)
{
	struct nct6694_wdt_dev *wdt = watchdog_get_drvdata(wdev);
	struct nct6694 *chip = wdt->data->chip;
	...
	ret = nct6694_setusb_wdata(chip, REQUEST_WDT_MOD, REQUEST_WDT_CMD0_OFFSET(wdt->wdev_idx), REQUEST_WDT_CMD0_LEN, buf);
	if (ret) {
...
static int nct6694_wdt_set_pretimeout(struct watchdog_device *wdev, unsigned int pretimeout)
{
	struct nct6694_wdt_dev *wdt = watchdog_get_drvdata(wdev);
	struct nct6694 *chip = wdt->data->chip;
	unsigned int timeout_fmt, pretimeout_fmt;
...
static unsigned int nct6694_wdt_get_time(struct watchdog_device *wdev)
{
	...
	ret = nct6694_getusb(chip, REQUEST_WDT_MOD, REQUEST_WDT_CMD0_OFFSET(wdt->wdev_idx), REQUEST_WDT_CMD0_LEN, WDT_COUNTDOWN_IDX, WDT_COUNTDOWN_LEN, buf);
	if (ret) {
...
static int nct6694_wdt_setup(struct watchdog_device *wdev)
{
	...
	pr_info("Setting WDT(%d): timeout = %d, pretimeout = %d", wdt->wdev_idx, wdev->timeout, wdev->pretimeout);
...
static int nct6694_wdt_probe(struct platform_device *pdev)
{
	...
	data = kzalloc(sizeof(struct nct6694_wdt_data), GFP_KERNEL);
	if (!chip)
		return -ENOMEM;
	...
	for (i = 0; i < data->nr_wdev; i++) {
		...
		if (ret) {
			dev_err(&pdev->dev, "Failed to register watchdog deivce: %d", ret);
			return ret;
		}
