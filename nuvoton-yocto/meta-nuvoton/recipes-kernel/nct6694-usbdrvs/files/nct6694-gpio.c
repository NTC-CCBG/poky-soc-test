...
static int nct6694_get_direction(struct gpio_chip *gpio, unsigned offset)
{
	struct nct6694_gpio_bank *bank = gpiochip_get_data(gpio);
	struct nct6694_gpio_data *data = bank->data;
	unsigned char ret, buf;

	ret = nct6694_getusb(data->chip, REQUEST_GPIO_MOD, GPO_DIR_REG + bank->group, REQUEST_GPIO_LEN, 0, 1, &buf);
	if (ret < 0) {
...
static int nct6694_direction_input(struct gpio_chip *gpio, unsigned offset)
{
	struct nct6694_gpio_bank *bank = gpiochip_get_data(gpio);
	struct nct6694_gpio_data *data = bank->data;
	unsigned char ret, buf;
	...
	buf &= ~(1 << offset);
	ret = nct6694_setusb_wdata(data->chip, REQUEST_GPIO_MOD, GPO_DIR_REG + bank->group, REQUEST_GPIO_LEN, &buf);
	if (ret < 0) {
		...
		unsigned offset, int val)
{
	...
	ret = nct6694_getusb(data->chip, 0xFF, GPO_DATA_REG + bank->group, REQUEST_GPIO_LEN, 0, 1, &buf);
	if (ret < 0) {
		pr_err("%s: Failed to get data from usb device!\n", __func__);
		return -EINVAL;
	}
	if (val)
		...
		buf &= ~(1 << offset);
	ret = nct6694_setusb_wdata(data->chip, REQUEST_GPIO_MOD, GPO_DATA_REG + bank->group, REQUEST_GPIO_LEN, &buf);
	if (ret < 0) {
		pr_err("%s: Failed to set data to usb device!\n", __func__);
		return -EINVAL;
	}
...
static int nct6694_get_value(struct gpio_chip *gpio, unsigned offset)
{
	struct nct6694_gpio_bank *bank = gpiochip_get_data(gpio);
	struct nct6694_gpio_data *data = bank->data;
	unsigned char ret, buf;
	...
	if (BIT(offset) & buf) {
		ret = nct6694_getusb(data->chip, REQUEST_GPIO_MOD, GPO_DATA_REG + bank->group, REQUEST_GPIO_LEN, 0, 1, &buf);
		if (ret < 0) {
			pr_err("%s: Failed to get data from usb device!\n", __func__);
			return -EINVAL;
		}
	...
	ret = nct6694_getusb(data->chip, REQUEST_GPIO_MOD, GPI_DATA_REG + bank->group, REQUEST_GPIO_LEN, 0, 1, &buf);
	if (ret < 0) {
		pr_err("%s: Failed to get data from usb device!\n", __func__);
		return -EINVAL;
	}
...
static void nct6694_set_value(struct gpio_chip *gpio, unsigned offset, int val)
{
	struct nct6694_gpio_bank *bank = gpiochip_get_data(gpio);
	struct nct6694_gpio_data *data = bank->data;
	unsigned char ret, buf;
							  ...
							  unsigned long config)
{
	...
	ret = nct6694_getusb(data->chip, REQUEST_GPIO_MOD, GPO_TYPE_REG + bank->group, REQUEST_GPIO_LEN, 0, 1, &buf);
	if (ret < 0) {
		pr_err("%s: Failed to get data from usb device!\n", __func__);
	}
	...
	ret = nct6694_setusb_wdata(data->chip, REQUEST_GPIO_MOD, GPO_TYPE_REG + bank->group, REQUEST_GPIO_LEN, &buf);
	if (ret < 0) {
		pr_err("%s: Failed to set data to usb device!\n", __func__);
	}
								    ...
								    unsigned int ngpios)
{
	...
	ret = nct6694_getusb(data->chip, REQUEST_GPIO_MOD, GPIO_VALID_REG + bank->group, REQUEST_GPIO_LEN, 0, 1, &buf);
	if (ret < 0) {
		pr_err("%s: Failed to get data from usb device!\n", __func__);
	}
	*valid_mask = buf;
...
static void nct6694_irq(struct work_struct* irq_work)
{
	...
	ret = nct6694_getusb(data->chip, REQUEST_GPIO_MOD, GPI_STS_REG, 16, 0, 16, status);
	if (ret < 0) {
		pr_err("%s: Failed to get data from usb device!\n", __func__);
	}

	for (int i = 0; i < GPIO_BANK_NR; i++) {
		u8 stat = status[i];
		...
		while (stat) {
			...
			stat &= ~(1 << bit);
			ret = nct6694_setusb_wdata(data->chip, REQUEST_GPIO_MOD, GPI_CLR_REG + i, REQUEST_GPIO_LEN, &val);
			if (ret < 0) {
				pr_err("%s: Failed to set data to usb device!\n", __func__);
			}
		}
...
void nct6694_start_irq(void)
{
	struct nct6694_gpio_bank *bank = nct6694_gpio_bank;
	struct nct6694_gpio_data *data = bank->data;
	queue_work(data->chip->async_workqueue, &data->irq_work);	
}

static int nct6694_irq_trig(struct nct6694_gpio_data *data)
{
	int ret;
	ret = nct6694_getusb(data->chip, REQUEST_GPIO_MOD, GPI_FALLING_REG,
...
static int nct6694_irq_set_type(struct irq_data *d, unsigned int type)
{
	...
	nct6694_setusb_async(data->chip, REQUEST_GPIO_MOD, GPI_FALLING_REG,
...
static int nct6694_gpio_probe(struct platform_device *pdev)
{
	...
	chip->gpio_hanlder = nct6694_start_irq;

	data = kzalloc(sizeof(struct nct6694_gpio_data), GFP_KERNEL);
	if (!data)
		return -ENOMEM;
